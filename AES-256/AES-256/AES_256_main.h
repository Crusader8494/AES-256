...
public:
	...
	AES(std::vector<uint8_t> inputKey, bool mathOrLUTGFMult) : mathOrLUTGFMultiplication(mathOrLUTGFMult)//Constructor
	{
		if (inputKey.size() != 32)
		{
			std::printf("Invalid Key Length \n");
			throw 1;
		}
		...
		//Generate Map which relates multiplier values to array indecies
		GFLUTMap[0x02] = 0x00;
		GFLUTMap[0x03] = 0x01;
		GFLUTMap[0x09] = 0x02;
		GFLUTMap[0x0B] = 0x03;
...
private:
	...
	const bool mathOrLUTGFMultiplication = true;

	uint8_t initialKey[4][8];
	uint8_t initialAndExpandedKey[4][8][15];
	...
	const uint8_t forwardMixColumnsMatrixPreFlipped[4][4] = {
		{0x02,	0x01,	0x01,	0x03},
		{0x03,	0x02,	0x01,	0x01},
		{0x01,	0x03,	0x02,	0x01},
		{0x01,	0x01,	0x03,	0x02}
	...
	const uint8_t inverseMixColumnsMatrixPreFlipped[4][4] = {
		{0x0E,	0x09,	0x0D,	0x0B},
		{0x0B,	0x0E,	0x09,	0x0D},
		{0x0D,	0x0B,	0x0E,	0x09},
		{0x09,	0x0D,	0x0B,	0x0E}
	...
	std::map<uint8_t, uint8_t> GFLUTMap;

	const uint8_t multiplyByXGFLUT[6][256] = {
		{
	...
	void SBoxState(bool forwardInverse);
	uint8_t SBoxByValue(bool forwardInverse, uint8_t value);
